...
The `home-routes` handler in the `not-so-secure-webapp.routes.home` namespace
...
The `home-page` function will in turn call the `not-so-secure-webapp.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/not_so_secure_webapp/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `not-so-secure-webapp.handler` namespace:
...
Request middleware functions are located under the `not-so-secure-webapp.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `not-so-secure-webapp.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `not-so-secure-webapp.db.core` in some other namespace.
